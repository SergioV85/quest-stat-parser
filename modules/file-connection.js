...
exports.createLogFile = (gameId) => {
  ...
  fs.writeFile(`/tmp/monitoring-${gameId}.csv`, opts)
    .catch((err) => {
      throw err;
    });
};
...
exports.saveMonitoringToFile = (gameId, entry) => {
  ...
  fs.appendFile(`/tmp/monitoring-${gameId}.csv`, csv)
    .catch((err) => {
      throw err;
    });
};
...
exports.saveMonitoringToS3 = (gameId) =>
  fs.readFile(`/tmp/monitoring-${gameId}.csv`)
    .then((fileBuffer) => {
...
exports.parseSavedLogs = (gameId) => {
  ...
  return readFromS3withPromise(params)
    .then((response) => parseCSV(response.Body.toString('utf-8')));
};
