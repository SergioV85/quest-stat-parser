...
export const monitoringCodesHandler: APIGatewayProxyHandler = async (event) => {
  const gameId = pipe(path(['queryStringParameters', 'gameId']), parseInt)(event);
  const levelId = pipe(path(['queryStringParameters', 'levelId']), parseInt)(event);
  const playerId = pipe(path(['queryStringParameters', 'playerId']), unless(isNil, parseInt))(event);
  const teamId = pipe(path(['queryStringParameters', 'teamId']), unless(isNil, parseInt))(event);
